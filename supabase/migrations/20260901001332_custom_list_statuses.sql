-- Replaces the fixed two-stage To Do/Done system on custom lists with
-- fully custom, per-list, user-defined statuses (any number, any names,
-- any order). Non-destructive: the old status_todo_label/status_done_label
-- columns on custom_lists and the status text column on custom_list_items
-- are left in place (unused going forward) rather than dropped.
...
-- Items now reference a status row instead of a fixed 'todo'/'done' enum.
-- ON DELETE SET NULL so deleting a status doesn't delete its items - the
-- app reassigns items to a fallback status before actually deleting one.
...
-- Backfill: every existing list gets exactly two statuses seeded from its
-- current status_todo_label/status_done_label (in that order), and every
-- existing item gets pointed at the matching one - so existing lists look
-- and behave identically the moment this ships.
DO $$
DECLARE
  list_row RECORD;
  todo_id UUID;
