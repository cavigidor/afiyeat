...
CREATE POLICY "Users can view their own list types" ON public.custom_list_types FOR SELECT USING (auth.uid() = user_id);
CREATE POLICY "Users can insert their own list types" ON public.custom_list_types FOR INSERT WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Users can update their own list types" ON public.custom_list_types FOR UPDATE USING (auth.uid() = user_id);
CREATE POLICY "Users can delete their own list types" ON public.custom_list_types FOR DELETE USING (auth.uid() = user_id);
...
-- 0-10 rating).
UPDATE public.custom_lists SET
  show_price = (value_field = 'price'),
