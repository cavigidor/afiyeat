...
interface SheetContentProps
  ...
  // Lets a consumer that builds its own header row (title + close button
  // together, e.g. ManageTypesSheet) suppress this default absolutely-
  // positioned close button instead of ending up with two of them.
  ...
  ({ side = "right", className, children, showCloseButton = true, ...props }, ref) => (
    <SheetPortal>
      ...
      <SheetPrimitive.Content ref={ref} className={cn(sheetVariants({ side }), className)} {...props}>
        {children}
        {showCloseButton && (
          <SheetPrimitive.Close className="active-press absolute right-4 top-[calc(1rem+env(safe-area-inset-top,0px))] rounded-sm opacity-70 ring-offset-background transition-[opacity,transform] duration-150 ease-out data-[state=open]:bg-secondary hover:opacity-100 active:opacity-50 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none">
            <X className="h-4 w-4" />
